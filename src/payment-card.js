...
export class Card extends EventTarget {
	...
	setDomElements() {
		this.elements = {
			pan: HtmlElement.bySelector("input[name=pan]"),
		...
		this.overlays = {
			pan: HtmlElement.bySelector("div#st-pan-overlay"),
	...
	setAutocomplete() {
		this.elements.pan.setAttributes({
		...
		this.elements.securitycode.setAttributes({
			"autocomplete": "cc-csc",
			"tabindex": "0"
		});
		this.elements.nameoncard.setAttributes({
	...
	addAutofillElement(name, type, parent) {
		var autofillElement = new HtmlElement("input", {
			type: "number",
			autocomplete: type,
			class: "autofill-input",
			tabindex: "-1"
		}); // Note we don't add the name attribute to this so we don't ever submit the expirymonth/year
