...
var _cardtype = __webpack_require__(18);

var _htmlelement = __webpack_require__(19);

var _utils = __webpack_require__(20);

var _binlookup = __webpack_require__(21);
...
function () {
  function Card(config) {
    ...
    this.config = config;
    this.template = _template.default;
    this.logos = {
      AMEX: _AMEXS.default,
  ...
  _createClass(Card, [{
    ...
    value: function getAllCardTypes() {
      ...
      for (var i in _cardtype.cardtypedetails) {
        result.push(_cardtype.cardtypedetails[i].type);
      }
    ...
    value: function getCard(type) {
      ...
      for (var i in _cardtype.cardtypedetails) {
        var card = _cardtype.cardtypedetails[i];
    ...
    value: function getMaxEntryLimits() {
      ...
      var panLimits = [];
      var cvcLimits = [];
      this.getAllCardTypes().forEach(function (type) {
        var card = _this.getCard(type);

        if (_this.isSupported(type)) {
    ...
    value: function isSupported(cardType) {
      return cardType in this.logos && (0, _utils.inArray)(this.config.supported, cardType);
    }
  }, {
    key: "shouldCenter",
    ...
    value: function updatePan() {
      ...
      if (newDetails !== this.cardDetails) {
        ...
        if (cardType !== null && this.isSupported(cardType)) {
          // TODO we've moved minMatch into binlookup, do we want to do the same with isSupported?
    ...
    value: function formatInput(type) {
      ...
      if (type == "pan") {
        ...
        if (format && value.length > 0) {
          // TODO we now could have a cardType without having any pan, Bad JuJu when we try to format a blank string
  ...
  _createClass(BinLookup, [{
    ...
    value: function binLookup(number) {
      ...
      if (number.length >= this.minMatch) {
        result = exports._lookup(number, _cardtype.cardtype); // eslint-disable-line no-undef
      }
      ...
      return _cardtype.cardtypedetails[result] || {
        type: null
      };
    }
  }]);
...
function binlookup(number) {
  return _cardtype.cardtypedetails[exports._lookup(number, _cardtype.cardtype)] || {
    type: null
  }; // eslint-disable-line no-undef
}

function _lookup(number, tree) {
  ...
  for (var key in tree) {
    if (exports.matchKey(number, key)) {
      // eslint-disable-line no-undef
      found.push(key);
    }
  }
