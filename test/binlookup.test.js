...
each([
          ...
          [{
               ...
               supported: ["AMEX", "MASTERCARD"]
          }, {
               default: cardtypedetails["4"],
               supported: ["AMEX", "MASTERCARD"]
          }],
          [{
               defaultCardType: "AMEX",
     ...
     .test("BinLookup.constructor", // Check different options get set on config correctly
          (testConfig, expected) => {
               if (expected instanceof Object) {
                    ...
                    expect(() => {
                         new binlookup.BinLookup(testConfig)
                    }).toThrow(expected);
               }
          });
...
test("BinLookup.getAllCardTypes",
     () => {
          ...
          expect(bl.getAllCardTypes()).toEqual(["AMEX", "ASTROPAYCARD", "DINERS", "DISCOVER", "JCB", "MAESTRO", "MASTERCARD", "PIBA", "VISA"]);
     });
...
each([
          ...
          [{
               type: "MASTERCARD"
          }, true], // we can pass the whole cardType object too
          ["", false],
          [undefined, false],
          [null, false],
          [{}, false],
     ])
     .test("BinLookup.isSupported",
          (cardType, expected) => {
               const bl = new binlookup.BinLookup();
...
each([
     ...
     [{
          defaultCardType: "MASTERCARD"
     }, "", null, cardtypedetails["2"]],
     [{
          defaultCardType: "AMEX",
          minMatch: 3
     }, "34", "1", cardtypedetails["4"]],
     [{
          defaultCardType: "AMEX",
          supported: ["AMEX"]
     }, "", "1", cardtypedetails["4"]],
...
]).test("BinLookup._lookup_withDefaults",
